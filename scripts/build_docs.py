"""Build self-contained HTML docs (wiki/html/*.html) from the markdown sources. Run from Project-V after any doc change:
.venv/bin/python scripts/build_docs.py
Sources: wiki/SPEC.md, wiki/*.md, every Video-*/RUNLOG.md (+ qc-report.md), wiki/html-src/{style.css,index.body.html}.
...
] + [(d.lower() + ".html", d, [f"{d}/RUNLOG.md"] + ([f"{d}/qc-report.md"] if os.path.isfile(f"{d}/qc-report.md") else [])) for d in VIDEO_DIRS]
...
for out, title, srcs in PAGES:
    ...
    open(f"wiki/html/{out}", "w").write(shell(title, body, out))
    print("wrote wiki/html/" + out)
vids = "".join(f'<div class="card"><h3><a href="{d.lower()}.html">{d}</a></h3><p>Run log and QC report for this video.</p></div>' for d in VIDEO_DIRS)
idx = open("wiki/html-src/index.body.html").read().replace("<!--VIDEOS-->", vids)
